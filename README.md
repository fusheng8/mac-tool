...
当前稳定版本为 `0.2.7`，仅支持 Apple Silicon（arm64）和 macOS 13 Ventura 或更高版本。隐私、故障恢复和兼容性说明分别见 [PRIVACY.md](PRIVACY.md)、[docs/recovery.md](docs/recovery.md) 与 [docs/compatibility.md](docs/compatibility.md)。
...
- Apple Silicon（arm64）；0.2.7 不提供 Intel 构建。
...
0.2.7 自动发布仍使用固定 Apple Development 身份和 Hardened Runtime；本地构建脚本已支持 Developer ID Application 与 Apple 公证，但不会自动改变 GitHub Release 门槛。安装后的版本更新由 Sparkle 使用 EdDSA 签名校验。发布前需要把本机导出的 Sparkle 私钥配置到仓库 Secret：
...
gh release create 0.2.7 .build/MacTool.dmg \
  --title 0.2.7 --generate-notes --verify-tag --draft
```
